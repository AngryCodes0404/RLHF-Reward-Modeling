...
class SlicPairPMPipeline:
    ...
    def __call__(self, candidates_A: List[str], candidates_B: List[str], **kwargs):
        ...
        for i in trange(len(candidates_A)):
            ...
            context = self.tokenizer_data_format.apply_chat_template(
                chosen[:-1], tokenize=False
            )
            ...
            for chosen_position in [0, 1]:
                ...
                input_ids = self.tokenizer.encode(
                    self.tokenizer.apply_chat_template(message, tokenize=False).replace(
                ...
                Z = np.exp(logit_A / self.temperature) + np.exp(
                    logit_B / self.temperature
                )
...
def get_token_count(example):
    ...
    context = pipeline.tokenizer_data_format.apply_chat_template(
        chosen[:-1], tokenize=False
    )
    responses = [chosen[-1]["content"], rejected[-1]["content"]]
    ...
    response_A = responses[chosen_position]
    response_B = responses[1 - chosen_position]
    prompt = pipeline.prompt_template.format(
        context=context, response_A=response_A, response_B=response_B
    )
    message = [
        {"role": "user", "content": prompt},
    ...
    input_ids = pipeline.tokenizer.encode(
        pipeline.tokenizer.apply_chat_template(message, tokenize=False).replace(
...
print(
    f"Data shard of {len(ds_subset)} examples, max length: {lengths.max()}, mean length: {lengths.mean()}"
)
ds_subset = ds_subset.filter(lambda x: x["length"] <= max_len)
print(
    f"Filtered examples of token length > {max_len}, remaining {len(ds_subset)} examples"
)

print("Data preprocessing done. Start pseudo-labeling.")
# get the pseudo-labels
preferences, chosen_probs = pipeline(
    candidates_A=ds_subset["chosen"],
    candidates_B=ds_subset["rejected"],
)
